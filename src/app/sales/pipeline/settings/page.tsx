...
export default async function SalesPipelineSettingsPage() {
  ...
  return (
    <>
      ...
      {!operationsConfigured ? <div className="sales-alert"><strong>CRM pendiente de migración.</strong> Aplica la migración 006 para guardar esta configuración.</div> : null}
        ...
        {stages.map((stage) => (
          <form action={updatePipelineStageAction} className={styles.stage} key={stage.key}>
            ...
            <div className={styles.field}>
              ...
              <input id={`label-${stage.key}`} name="label" defaultValue={stage.label} disabled={!operationsConfigured} />
              <span className={styles.key}>{stage.key}{stage.terminal ? " · etapa terminal" : ""}</span>
            </div>
            <div className={styles.field}>
              <label htmlFor={`position-${stage.key}`}>Orden</label>
              <input id={`position-${stage.key}`} name="position" type="number" min="0" step="1" defaultValue={stage.position} disabled={!operationsConfigured} />
            </div>
            <div className={styles.field}>
              <label htmlFor={`probability-${stage.key}`}>Probabilidad %</label>
              <input id={`probability-${stage.key}`} name="probability" type="number" min="0" max="100" step="1" defaultValue={stage.probability} disabled={!operationsConfigured} />
            </div>
            <label className={styles.checkbox}>
              <input type="checkbox" name="active" defaultChecked={stage.active} disabled={!operationsConfigured} />
              <span>Visible</span>
            </label>
            <button className={styles.save} type="submit" disabled={!operationsConfigured}>Guardar etapa</button>
          </form>
        ))}
      </div>
    </>
