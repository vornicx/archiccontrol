...
export default async function SalesTodayPage() {
  const { leads, persistenceConfigured } = await getSalesData();
        ...
        <div className="sales-task-list">
          {anteroQueue.map((lead, index) => (
            <Link className="sales-task" href={`/sales/leads/${lead.id}`} key={lead.id}>
              ...
              <div><span className="sales-badge" data-stage={lead.stage}>{salesStageLabels[lead.stage]}</span><h3>{lead.name}</h3><p>{lead.city || "—"} · {lead.category || "Negocio"}{lead.estimatedValue ? ` · ${money.format(lead.estimatedValue)}` : ""}</p></div>
      ...
      <section className="sales-section">
        ...
        <div className="sales-task-list">
          {active.filter((lead) => lead.nextActionOwner === "vadim" && lead.nextAction).map((lead) => (
            <Link className="sales-task" href={`/sales/leads/${lead.id}`} key={lead.id}>
              <span className="sales-rank">→</span><div><span className="sales-badge" data-stage={lead.stage}>{salesStageLabels[lead.stage]}</span><h3>{lead.name}</h3><p>{lead.city || "—"} · {lead.category || "Negocio"}</p></div><div className="sales-next"><strong>{lead.nextAction}</strong><span>Vadim</span></div><span className="sales-score">{lead.score ?? "—"}</span>
            </Link>
          ))}
        </div>
      </section>
