...
export function buildPrototypeFiles(candidate: ResearchCandidate, heroImage: string | null): PrototypeFile[] {
  const data = {
    ...
    copy: candidate.copy,
    heroImage: cleanImageUrl(heroImage),
  };
...
export default function Home() {
  const heroStyle = data.heroImage
    ? { backgroundImage: "linear-gradient(90deg, rgba(8,8,8,.92), rgba(8,8,8,.35)), url('" + data.heroImage + "')" }
    : undefined;
  ...
  return (
    <main>
        ...
        <div className="hero-grid">
          <div className="hero-copy">
            ...
            <div className="hero-actions">
              ...
              <a className="button" href="#services">Explore</a>
            </div>
          </div>
          <div className="hero-meta"><span>{data.category}</span><span>{data.city}</span></div>
        </div>
      </section>

      <section className="intro shell" id="services">
        <span className="index">01</span>
        <div><p className="eyebrow">Selected services</p><h2>Designed around the experience, not the transaction.</h2></div>
      </section>

      <section className="service-grid shell" aria-label="Services">
      ...
      <section className="story shell" id="story">
        <div><p className="eyebrow">The experience</p><span className="index">02</span></div>
        <div className="story-copy"><h2>{data.copy.storyTitle}</h2><p>{data.copy.storyBody}</p><p className="muted">{data.summary}</p></div>
      </section>
      ...
      <footer className="footer shell"><span>{data.name}</span><span>{data.city}</span><span>Concept by Archic</span></footer>
    </main>
  );
}
`;
...
.nav-shell { position: fixed; inset: 0 0 auto; z-index: var(--prototype-nav-layer); min-height: 82px; display: flex; align-items: center; justify-content: space-between; gap: 24px; padding: 0 clamp(22px,4vw,64px); border-bottom: 1px solid rgba(255,255,255,.10); background: linear-gradient(180deg,rgba(4,4,4,.84),rgba(4,4,4,.20)); backdrop-filter: blur(16px); }
...
.hero-grid,.shell { width: min(var(--max),100%); margin-inline: auto; padding-inline: clamp(22px,5vw,78px); }
.hero-grid { padding-top: 170px; padding-bottom: 64px; display: grid; grid-template-columns: minmax(0,1fr) auto; gap: 40px; align-items: end; }
...
.hero-meta { display: grid; gap: 6px; color: rgba(243,240,232,.62); font-size: 10px; letter-spacing: .14em; text-align: right; text-transform: uppercase; }
...
.index { color: var(--muted); font-family: Georgia,serif; font-size: 14px; }
...
@media (max-width:820px) { .nav-shell nav>a:not(.nav-cta){display:none}.hero-grid{grid-template-columns:1fr}.hero-meta{display:none}.intro{grid-template-columns:1fr;padding-top:90px;padding-bottom:90px}.service-grid{grid-template-columns:1fr}.service-card,.service-card:nth-child(3n){min-height:210px;border-right:0}.story{grid-template-columns:1fr;min-height:auto}.footer{grid-template-columns:1fr;gap:8px;padding-block:34px} }
